...
BUTTERFLY_train_nonnorm = datasets.ImageFolder(root="./data/train/", transform=transforms.Compose([transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)), transforms.ToTensor()]))
...
BUTTERFLY_train = datasets.ImageFolder(root="./data/train/", transform=BUTTERFLY_transform)
BUTTERFLY_val = datasets.ImageFolder(root="./data/valid/", transform=BUTTERFLY_transform)
BUTTERFLY_test = datasets.ImageFolder(root="./data/test/", transform=BUTTERFLY_transform)
...
def train(model, loader, num_epoch = num_epoch): # Train the model
    ...
    plt.plot(epochs, epoch_losses, label="Loss", marker='o')
    plt.plot(epochs, epoch_accuracies, label="Accuracy", marker='s')
...
def imgae_grad(model): 
    model.eval()
    image_path = './data/train/ADONIS/001.jpg'
    input_image = Image.open(image_path).convert("RGB")
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Resize to model's input size
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Use ImageNet's normalization
    ])
    input_tensor = transform(input_image).unsqueeze(0)  # Add batch dimension

    # Initialize Grad-CAM
    target_layer = 'layer4'  # Target the last convolutional layer
    cam_extractor = GradCAM(model, target_layer)

    # Forward pass to compute the CAM
    output = model(input_tensor)
    class_idx = output.argmax(dim=1).item()  # Get the predicted class index
    activation_map = cam_extractor(class_idx, output)  # Compute Grad-CAM for the target class

    # Visualize the CAM
    # Convert the image to a NumPy array for visualization
    input_image_np = cv2.cvtColor(np.array(input_image), cv2.COLOR_RGB2BGR)
    heatmap = activation_map[0].cpu().numpy()  # Extract the heatmap
    result = overlay_mask(input_image_np, heatmap, alpha=0.5, colormap=cv2.COLORMAP_JET)

    # Display the result
    plt.figure(figsize=(8, 8))
    plt.imshow(cv2.cvtColor(result, cv2.COLOR_BGR2RGB))  # Convert to RGB for Matplotlib
    plt.axis('off')
...
def evaluate(model, loader): # Evaluate accuracy on validation / test set
    ...
    acc = correct/len(loader.BUTTERFLY_train_nonnorm )
    print("Evaluation accuracy: {}".format(acc))

    topFiveAcc = topFiveCorrect/len(loader.BUTTERFLY_train_nonnorm)
